...
$(function  () {
	...
	//点击关闭按钮 继续下载应用
	$(".close-bt,.i-now-bt").click( function(){
		hidens(this);
	});

	//什么是vivo帐号
	$(".title-right").click(function  () {
		showDiv("#what-v-account");
	})
	...
	if(!placeholderSupport()){ 
		$.each($('[placeholder]'),function  (i,val) {
			...
			if (val.type=="password") { 
				$(val).before('<input class="pwdPlaceholder" type="text"  value='+val.placeholder+' />'); 
				$(val).hide();
